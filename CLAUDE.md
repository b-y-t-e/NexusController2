# Nexus Controller â€” notes for Claude

Android phone â†’ virtual Xbox 360 / DualShock 4 / Buzz! controller on Windows.
...
and needs neither a driver nor a phone â€” keep new logic on the pure side.
...
* **Wire Y axis is "+ = up."** The only inversion lives in `DualShock4Pad`; DS4
  hardware runs 0 = up â€¦ 255 = down. Do not add a second flip elsewhere.
* **`-128` is never a legal axis value.** Clamp to `-127`.
* **Buzz clients send *semantic* bits** (Red=0x01 â€¦ Blue=0x10); the **server**
  translates to XInput. Never pre-map on the client.
* **Layout coordinates are normalised 0â€“1 fractions of the screen**, addressing a
  component's centre â€” never pixels. Central config from the PC depends on it.
...
* Every `sendall` on a client socket goes through `PlayerSession.send()` â€” the
  rumble callback thread and the reader thread both write.

## Gotchas discovered the hard way

* **Windows `SO_REUSEADDR` lets another process steal a bound port** â€” it is not
...
* `Modifier.scale`/`rotate` are graphicsLayers and Compose *does* transform
  pointer coordinates, so a gesture inside them receives local-space deltas â€”
...
* Tests are the deliverable, not an afterthought. New protocol behaviour needs a
  ...
  vectors hand-written â€” never generate them from the code under test.
...
* Binaries do not go in git â€” they are built and attached to GitHub Releases.
* Commit messages in Polish, code and comments in English (the UI is English too).

## Roadmap

**Done** â€” protocol v2 with token pairing; Xbox/DS4/Buzz emulation; 8 players
(`protocol.MAX_PLAYERS`; only 4 of them can be XInput-backed â€” see `xinput.py`);
rumble; offline dashboard; key bindings; XInput capacity detection; 532 server
tests + 114 Kotlin tests + hardware smoke test.

**Done, continued** â€” central configuration (`PROTOCOL.md` Â§10): live pad preview
...
* Native Buzz HID device emulation â€” RPCS3 drives buzzers from ordinary pads, so
